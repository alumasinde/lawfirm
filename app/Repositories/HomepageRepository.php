...
final class HomepageRepository
{
    ...
    {
        $keys = array_values(array_filter(array_unique(array_map('strval', $keys)), static fn (string $key): bool => $key !== ''));
        if ($keys === []) return [];
        ...
        $sections = [];
        foreach ($rows as $row) $sections[(string) $row['section_key']] = $row;

        return $sections;
    }

    public function section(string $key): ?array { return $this->sections([$key])[$key] ?? null; }
    ...
    {
        return $this->database->statement(
            'SELECT * FROM practice_areas WHERE is_enabled = 1 AND is_featured = 1
             ORDER BY sort_order ASC, id ASC LIMIT ' . max(1, $limit)
        )->fetchAll();
    }
    ...
    {
        return $this->database->statement(
            'SELECT a.*, m.path AS photo_path FROM advocates a
             LEFT JOIN media m ON m.id = a.photo_media_id
             WHERE a.is_enabled = 1 AND a.is_featured = 1
             ORDER BY a.sort_order ASC, a.id ASC LIMIT ' . max(1, $limit)
        )->fetchAll();
    }
